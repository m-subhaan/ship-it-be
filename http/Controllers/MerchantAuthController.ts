...
class MerchantAuthController {
  static async login(request: TRequest, response: TResponse) {
    try {
      ...
      const httpResponse = await authService.login(loginDTO, LOGIN_TYPES.MERCHANT);
  ...
  static async forgotPassword(request: TRequest, response: TResponse) {
    try {
      ...
      const httpResponse = await authService.forgotPassword(forgotPasswordDTO, LOGIN_TYPES.MERCHANT);
  ...
  static async resetPassword(request: TRequest, response: TResponse) {
    try {
      ...
      const httpResponse = await authService.resetPassword(resetPasswordDTO, LOGIN_TYPES.MERCHANT);
...
export default MerchantAuthController;
